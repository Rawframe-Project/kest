...
| `docs/compact.md` | The whole language in one sitting, for a model or anybody who has to write it today: every rule a program meets, short, with every program on it run by `check-docs.sh` and the standard library's names held to the library. It says what; `docs/language.md` is what it is held to. See D1259. |
...
bench/             Four shapes of work and what each costs, in this language
                   ...
                   not there. Two of its rows are of something built before
                   it is run — this language's release engine, and daslang's
                   own compiler writing a binary — and the building is not in
                   the duration, because it is not what either of them does
                   when a game runs. It says what a row holds before any of the
...
tools/             Build and development scripts. `make check` runs all of
                   ...
                   being something: a backend that wrote no body would hand
                   the host's compiler a file of comments and pass. Writing
                   every body used to be wrong here too, because there was no
                   C for a crossing into the host; there is now, and a program
                   wholly written is the ordinary case.
                   ...
                   frame that says nothing, about a constant that cannot be
                   worked out and is refused by nobody, about a function value
                   that stands for nothing, about an escape nothing
                   ...
                   tests let the wrong answer through, about a store read a
                   slot short of what it holds,
                   ...
                   about a program written for the range and not weighed,
                   ...
                   as a question nobody asked, about a call back in that starts
                   from nowhere, about what one function needs
                   answered with its own frame, about a reason from elsewhere
                   said to have come from here, about a caller of a function
                   with no answer that says it has one, about a walk that stops
                   at a
                   function in one form only, about a call that names the
                   first function rather than its own, about a module named by
                   where
                   the file is rather than by its name, about an edit as long
                   as
                   everything before it and itself, about a formatter that
                   answers
                   with what it was given, about a comment shown at a place
                   it was not written, about an entry point whose own
                   number is the whole program's, about a layout that says it
                   holds no tag, about an imported module summed
                   up without what a host provides, about a field at one byte
                   in the
                   words and another in the object, about a promise in one form
                   of
                   a listing and not the other, about a chunk that gives
                   something back where nothing is declared, about a run that
                   says nought
                   for an answer there is not, about a call that writes where
                   ...
                   was not one, about a question past the last
                   function that goes quiet, about the end of a walk that says
                   something, about a build that says it was
                   freed and kept the program, about a tick whose words say a
                   ...
                   stack than it was asked for, about a build that says it cost
                   nothing, about a run saying its own work
...
are both said about rather than read as whatever came last. And the other half
of that: a check that refuses says what is wrong, so one that refuses with
nothing to say is said about too — a word and no reason is what `failed = 1`
looks like under a condition whose complaints all found nothing. See D878.
...
threads      two machines of one build running at once on two threads, a
...
races        the same thing watched rather than argued about: four machines
             ...
             machine reads and one count in the process. And four more
             started, run and freed on four threads at once, which is the
             other half of what the reference says a host may do and the
             half the first four say nothing about: they are started and
...
reload       the edits a reload has to have an answer for, each driven
             ...
             the end of one, a bit put in the middle of a set of named ones,
             a bit added at the end of one, and a program that will not
             build. Every one of
             them ends with a world -- the new program's or the one the host
             was holding -- and never half of either. The four about a case
             and a bit are the ones the layout's mark said nothing about
             until D1072 and D1076: both are a place in a list, so one put
...
tree         nothing a compiler made in the tree but what `make clean` takes
             ...
             walk is right to allow in a working tree is one nothing should
             have put in the repository — and no file with a space in its
             name, which is what an unquoted redirect leaves and what every
...
block only in front of a brace, `own` marks a field only in front of a name
and a colon -- so a field called `own` still works -- and `compares` and
`orders` say what a generic asks of a type name, only after a colon inside
