...
and name, **for a tag which cases it can name and in what order, and for a set
of named bits which bits it has and in what order**. Both of those are a number
that is a place in a list: a case put in the middle of an enum renumbers every
case after it, a bit put in the middle of a set doubles every bit after it, and
in both the size, the alignment and every piece stay exactly where they were.
Neither was in the mark until D1072 and D1076, and a world saved before such an
edit read back with each case, and each bit, meaning the one below it. What is *not* promised is that a reference is an identity: a `ref<T>` is
...
55860 bytes as tokens, 136241 as a tree, 174952 checked and 207398 compiled.
...
file and 20701 bytes, against the 207398 it costs to compile.
...
numbers together say how much of that finding out answered: 113 of 507 for
