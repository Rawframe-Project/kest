...
reasoning; this holds the position.
    ...
    CURRENT SHA:       (this commit) D1093-D1252
    PHASE:             B — the release engine, and it is whole: every one of
                       ...
                       daslang's AOT on four, and 1.17 times behind it on
    ...
    LAST CI:           green on ae69897d, all ten jobs -- linux,
    ...
    LAST FULL GATE:    CI's `linux-full` on ae69897d, every hole caught; the
                       sweep here on D1252, 934 holes, every one caught
...
30. *(done, D1153)* **The colony on the held doors.** 744 lines of host to
...
31. *(done, D1154-D1178)* **The machine against Luau's interpreter.**
   ...
   A float local against a constant and a float sum of two locals made one
   instruction each took `kernel` 15.6% below where it was and under Luau's
   ...
   and a UTF-8 check over eight bytes at once 1.5 more, to 1.15 (D1169). A
   ...
   4.8%, to 1.01 times Luau's interpreter in instructions (D1177), and an
...
37. *(done, D1183)* **Coroutines (B) and the library (C), asked of the
   evidence.** Neither shown to be needed; each is written down with why.
36. *(done, D1182)* **A debugger in the editor.** `kest dap`, the Debug
   Adapter Protocol over the same debugger `kest debug` is, started by the
   VS Code extension: breakpoints, frames, variables, stepping and output.
...
32. *(open, D1161, D1179)* **Daslang in every mode it ships.** `graph`, `words` and
   ...
   fewer cycles (D1202).
   A question that jumps over a jump asks the other way: the colony's trial
   1.3 to 3.2% fewer cycles (D1205).
   A body whose every way out is a `return` in a `match` is written as C:
   the colony's trial compiles whole, 6% fewer instructions (D1206).
   A body with more than three arguments reads them out of the frame even
   in locals: the colony's trial compiled 9.5% fewer cycles (D1207).
...
and not built for three passes it would need -- built since, in D1231, at
5.2% of the trial's cycles. And a constant a carried body
