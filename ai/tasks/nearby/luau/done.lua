...
function nearby.tally(lines: { string }): Board
    ...
    for _, line in lines do
        ...
        -- `tonumber` reads a fraction as readily as a whole number, and a
        -- score with a fraction in it is not a score here.
        local worth = tonumber(trimmed(pieces[2]))
        if worth == nil or worth ~= math.floor(worth) then
            continue
        end
