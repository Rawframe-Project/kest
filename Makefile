...
# and the game is `examples/tetris/game.kest`. Not in any other target, for the
# reason the coverage fuzzer is not: raylib is somebody else's, built from its
# own source, and `RAYLIB` is where its `src` was built. CI's `tetris` job
# builds it and plays the game under a display nobody sees. See D1269.
RAYLIB ?=
examples/tetris/host: examples/tetris/host.c libkest.a include/kest.h
	@test -n "$(RAYLIB)" || \
...
clean:
	rm -rf build kest kest-debug libkest.a examples/embed \
	    ...
	    tools/fuzz-cover kest.wasm examples/tetris/host \
