...
reasoning; this holds the position.
    ...
    CURRENT SHA:       (this commit) D1093-D1155
...
31. *(open, D1154, D1155)* **The machine against Luau's interpreter.**
   Measured again: behind by a sixth to double, `rules` furthest. Seven things
   off it so far: `rules` is 20.6% fewer instructions and 1.39 times Luau's
   interpreter where it was 1.76; `control` -13.7%, `words` -15.2%, `graph`
   -8.5%.
