...
function pooled.refill(pool: Pool, want: number, into: { number }): number
    ...
    for _ = 1, want do
        local slot = pooled.take(pool)
        if slot == nil then
            -- What was taken this call goes back, newest first, so the pool
            -- ends holding what it held and in the order it held it.
            for i = #into, 1, -1 do
                pooled.give(pool, into[i])
            end
            table.clear(into)
            return 0
        end
        table.insert(into, slot)
    end
    return want
end
